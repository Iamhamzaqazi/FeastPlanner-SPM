...
{
    public partial class IncomeStats
    {
        #region Variables
        ...
        TrnsBusinessIncome oModel = new TrnsBusinessIncome();

        private IEnumerable<TrnsBusinessIncome> oList = new List<TrnsBusinessIncome>();
        private IEnumerable<TrnsBusinessIncome> oFilterList = new List<TrnsBusinessIncome>();

        private string searchString1 = "";
        private bool FilterFunc(TrnsBusinessIncome element) => FilterFunc(element, searchString1);

        #endregion

        #region Functions
        private bool FilterFunc(TrnsBusinessIncome element, string searchString1)
        {
        ...
        private async Task GetAllIncome()
        {
            try
            {
                string Clause = $@" AND BusinessKey = '{BusinessKey}' AND Date Between  '{Convert.ToDateTime(DateRange.Start).ToString("yyyy-MM-dd")}'
                                                                                    ...
                                                                                    AND IsActive = 'True'";
                oFilterList = await _trnsFinancial.GetAllIncomeData(Clause);
            }
            catch (Exception ex)
            {
